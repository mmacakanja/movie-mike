export default async function handler(req,res){
  ...
  if(req.method==='GET'){
    try{const r=await fetch(url); const text=await r.text(); res.setHeader('Content-Type','application/json'); return res.status(r.ok?200:502).send(text);}catch(e){return res.status(500).json({error:'Unable to read Google Sheet'});}
  }
  if(req.method==='POST'){
    const supplied=req.headers['x-admin-secret'];
    if(!process.env.REVIEW_ADMIN_SECRET || supplied!==process.env.REVIEW_ADMIN_SECRET)return res.status(401).json({error:'Invalid admin secret'});
    try{
