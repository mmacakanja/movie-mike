# Movie Mike — Version 4

Version 4 keeps the existing rating site, but makes the data plumbing much more durable and owner-friendly.
...
Version 4 adds **Newest Comments** near the top of the homepage. It displays the latest eight comments across all movies. Clicking one opens that movie.

This gives you an in-site notification/summary with no additional service. Email alerts are also possible later, but would require an outbound email provider (for example Resend) or another notification service, so V4 intentionally keeps the first notification system simple.
...
Upload all V4 files to the same GitHub repository and preserve the folder structure. Replace the old files with the V4 versions. In particular, make sure the repository root contains:
...
and that `api/` contains the V4 API files.
