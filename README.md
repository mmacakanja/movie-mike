...
Version 5 keeps Version 4's permanent cache, live ratings and comments, while making the public site clearer and adding guest columns and opt-in email updates.
...
- Subscriber signup, unsubscribe links, automatic new-movie email alerts, and a protected general-update sender.
...
### Required for subscriber email notifications

`RESEND_API_KEY`

Create this in Resend and store it as a secret in Vercel.

`UPDATE_FROM_EMAIL`

The verified sender used for update emails, for example `Movie Mike <updates@yourdomain.com>`. Resend requires the sending domain to be verified.

`SITE_URL`

Set this to `https://movie-mike.vercel.app` so email links and unsubscribe links point to the production site.

Subscriber addresses and confirmation/unsubscribe tokens are stored in Neon in the automatically created `update_subscribers` table. Subscribers must click an emailed confirmation link before updates begin. Adding a movie through Owner Tools sends a notification automatically. The expanded Owner Tools box also includes a protected form for announcing a guest column or another general site update.

## Publishing a Visiting Columnist

Open `columnist.js`, set `published: true`, and replace the author, title, date and paragraph text. Each item in the `paragraphs` array becomes a paragraph on the site. Set `published: false` to show the default invitation instead.
...
Version 5 adds optional email subscriptions through Resend. Without the three email environment variables above, the rest of the site continues to work normally.
...
and that `api/` contains the V5 API files, including `subscribe.js`, `confirm-subscription.js`, `unsubscribe.js`, `notify.js`, and `notify-update.js`.
