...
export default async function handler(req,res){
  ...
  if(req.method==='POST'){
    ...
    try{
      ...
      if(r.ok){
        const vals=['o','d','a','e','p'].map(k=>Number(req.body?.[k]));
        const score=vals.every(Number.isFinite)?vals.reduce((a,b)=>a+b,0)/5:null;
        try{await notifySubscribers({title:String(req.body?.title||'New Movie Mike rating').trim(),score})}catch(e){console.error('Subscriber notification failed',e)}
      }
