...
async function fetchMetadata(m){
  ...
  try{
    ...
    const r=await fetch(`/api/tmdb?title=${q}${y}`);if(!r.ok){if(r.status===503)metadataEnabled=false;metadataFailed.add(m.title);throw 0}
    const md=await r.json();metadata.set(m.title,md);metadataFailed.delete(m.title);loadedMetadata++;try{localStorage.setItem(cacheKey,JSON.stringify(md))}catch(e){}return md;
...
async function enrichBatch(list,rerender=true){
  const queue=[...list]; let completed=0;
  const workers=Array.from({length:6},async()=>{
    while(queue.length&&metadataEnabled){
      await fetchMetadata(queue.shift()); completed++; updateMetadataStatus();
      // Refresh visible cards periodically so posters appear as metadata arrives.
      if(rerender && completed%12===0){renderMovies(); renderPantheon(); renderRecent(); renderCollections(); renderHeroPosters();}
    }
  });
  await Promise.all(workers);
  populateGenreFilter(); renderGenreStats();
  // Always refresh once when a metadata pass finishes, including the background pass.
  renderMovies(); renderPantheon(); renderRecent(); renderCollections(); renderHeroPosters();
}
...
(async function init(){await fetchSheetMovies();renderAll();enrichBatch([...pantheon(),...recent(),...MOVIES.slice(0,80)].filter((m,i,a)=>a.findIndex(x=>x.title===m.title)===i));setTimeout(()=>enrichBatch(MOVIES.filter(m=>!metadata.has(m.title)),true),1200)})();
