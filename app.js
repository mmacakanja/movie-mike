...
function updateMetadataStatus(){
  ...
  if(!metadataEnabled){s.textContent='Movie details are temporarily unavailable.';return}
...
function renderGenreStats(){const c={};for(const md of metadata.values())for(const g of md?.genres||[])c[g]=(c[g]||0)+1;const rows=Object.entries(c).sort((a,b)=>b[1]-a[1]).slice(0,14);document.getElementById('genreStats').innerHTML=rows.length?rows.map(([g,n])=>`<button type="button" class="genre-chip" data-genre="${encodeURIComponent(g)}">${esc(g)} <strong>${n}</strong></button>`).join(''):'<span class="panel-note">Genre lists will appear as movie details load.</span>'}
...
function renderDisagreements(){
  const mikeEl=document.getElementById('mikeHigherGrid'),criticEl=document.getElementById('criticHigherGrid');if(!mikeEl||!criticEl)return;
  const rows=MOVIES.map(m=>{const c=criticData.get(m.title)?.metacritic;if(c==null)return null;const mike=Math.round(m.score/7*100);return {m,c,mike,gap:mike-c}}).filter(Boolean);
  const high=rows.filter(x=>x.gap>0).sort((a,b)=>b.gap-a.gap).slice(0,4),low=rows.filter(x=>x.gap<0).sort((a,b)=>a.gap-b.gap).slice(0,4);
...
function renderColumnist(){
  const c=window.VISITING_COLUMNIST;if(!c||!c.published)return;
...
async function submitSubscription(e){
  e.preventDefault();const form=e.target,msg=document.getElementById('subscribeMessage'),email=new FormData(form).get('email');
  msg.textContent='Subscribing…';
  try{const r=await fetch('/api/subscribe',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({email})}),d=await r.json();if(!r.ok)throw new Error(d.error||'Unable to subscribe');msg.textContent=d.alreadySubscribed?'You are already subscribed. No duplicate emails—civilization survives.':'Check your inbox and click the confirmation link.';form.reset()}
  catch(err){msg.textContent=err.message||'Subscriptions are temporarily unavailable.'}
}
...
async function loadNewestComments(){
  ...
  catch(e){el.innerHTML='<p class="panel-note">The comments are quiet for now.</p>'}
}
async function loadComments(title){
  ...
  catch(e){area.innerHTML='<p class="panel-note">Comments are unavailable right now.</p>'}
}
...
async function submitUpdateNotice(e){
  e.preventDefault();const form=e.target,fd=new FormData(form),msg=document.getElementById('notifyMessage');msg.textContent='Sending…';
  try{const r=await fetch('/api/notify-update',{method:'POST',headers:{'Content-Type':'application/json','x-admin-secret':fd.get('secret')},body:JSON.stringify({title:fd.get('title'),message:fd.get('message')})}),d=await r.json();if(!r.ok)throw new Error(d.error||'Unable to send');msg.textContent=d.configured===false?'Email service is not configured yet.':`Sent to ${d.sent} subscriber${d.sent===1?'':'s'}.`;if(d.configured!==false)form.reset()}
  catch(err){msg.textContent=err.message||'Unable to send update.'}
}
...
document.getElementById('subscribeForm').onsubmit=submitSubscription;
document.getElementById('notifyForm').onsubmit=submitUpdateNotice;
document.addEventListener('click',e=>{const genre=e.target.closest('[data-genre]');if(genre){const value=decodeURIComponent(genre.dataset.genre);document.getElementById('genreFilter').value=value;document.getElementById('searchInput').value='';visible=56;renderMovies();document.getElementById('explore').scrollIntoView({behavior:'smooth'});return}const card=e.target.closest('[data-title]');if(!card)return;const m=MOVIES.find(x=>x.title===decodeURIComponent(card.dataset.title));if(m)openMovie(m)});
...
(async function init(){
  renderColumnist();await fetchSheetMovies();renderAll();loadNewestComments();await loadCachedCritics();
